...
const GRAMMARS: Grammar[] = [
    {
        ...
        confidence: 0.82,
        color: "border-sky-400/40 bg-sky-400/8",
        accent: "bg-sky-500",
        evidence: [
    ...
    },
    {
        ...
        confidence: 0.88,
        color: "border-green-400/40 bg-green-400/8",
        accent: "bg-green-500",
        evidence: [
    ...
    },
    {
        ...
        confidence: 0.0,
        color: "border-muted/40 bg-muted/5",
...
function computeScores(input: SimInput): Record<string, number> {
    const lumma = input.hasDomainDetect ? 0.82 : 0.05
    const risepro = input.hasNetworkUserDirs ? 0.88 : 0.04
    const whitesnake = input.hasSystemDirs ? 0.95 : 0.03
    const redline = input.hasLogTxt ? 0.78 : 0.04
    return { lumma, risepro, whitesnake, redline, generic: 0.0 }
}
                    ...
                    <div className="flex flex-wrap gap-2">
                        {GRAMMARS.map((g) => (
                            <button
                                ...
                                className={`px-3 py-1.5 rounded-lg border text-xs font-mono transition-all duration-200 ${g.color} ${selected === g.id && !showSim ? "ring-2 ring-primary opacity-100" : "opacity-60 hover:opacity-90"}`}
                            >
                                {g.name}
                            </button>
                        ))}
                        <button
                            ...
                            className={`px-3 py-1.5 rounded-lg border text-xs font-mono transition-all duration-200 border-primary/40 bg-primary/10 text-primary ${showSim ? "ring-2 ring-primary opacity-100" : "opacity-60 hover:opacity-90"}`}
                        >
                            ▸ Simulate
                        </button>
                    </div>
                        ...
                        {!showSim ? (
                            ...
                            >
                                <div className="space-y-3">
                                    <div className={`flex items-center justify-between p-3 rounded-lg border ${sel.color}`}>
                                        <span className="font-mono font-semibold text-sm">{sel.name}</span>
                                        <div className="flex items-center gap-2">
                                            <div className="text-xs text-muted-foreground">confidence</div>
                                            <Badge variant="outline" className={`font-mono text-xs ${sel.confidence >= 0.8 ? "border-green-400/50 text-green-400" : sel.confidence >= 0.6 ? "border-yellow-400/50 text-yellow-400" : "border-muted/50 text-muted-foreground"}`}>
                                    ...
                                    <div className="space-y-1">
                                        ...
                                        <div className="h-2 rounded-full bg-muted/40 overflow-hidden">
                                            <motion.div
                                                ...
                                                className={`h-full rounded-full ${sel.accent}`}
                                            />
                                        </div>
                                        {sel.confidence > 0 && (
                                            <div className="flex items-center gap-1 text-xs text-green-400">
                                                <CheckCircle2 className="w-3 h-3" /> Above 0.60 threshold — wins routing
                                            </div>
                                        )}
                                        {sel.confidence === 0 && (
                                            <div className="flex items-center gap-1 text-xs text-muted-foreground">
                                                <XCircle className="w-3 h-3" /> Always fallback — activates when all others fail
                                            </div>
                                        )}
                                    </div>
                            ...
                            >
                                    ...
                                    {(
                                        ...
                                        <label key={key} className="flex items-center gap-2 text-sm cursor-pointer group">
                                            <div
                                                ...
                                                className={`w-5 h-5 rounded border-2 flex items-center justify-center transition-colors ${simInput[key] ? "bg-primary border-primary" : "border-muted-foreground/40"}`}
                                            >
                                                {simInput[key] && <CheckCircle2 className="w-3.5 h-3.5 text-primary-foreground" />}
                                            </div>
                                            ...
                                            <div key={g.id} className="space-y-1">
                                                ...
                                                <div className="h-1.5 rounded-full bg-muted/40 overflow-hidden">
                                                    <motion.div
                                                        ...
                                                        className={`h-full rounded-full ${isWinner ? g.accent : "bg-muted-foreground/30"}`}
                                                    />
                                                </div>
                                            </div>
                                        )
                                ...
                                <div className={`p-3 rounded-lg border text-sm ${winnerGrammar.color}`}>
