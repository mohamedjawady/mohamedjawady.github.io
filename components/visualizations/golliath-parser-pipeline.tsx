    ...
    const stages: Stage[] = [
        ...
        },
        {
            ...
            how: [
                ...
                "RedLineGrammar: log.txt + Passwords/ co-presence (score ~0.78)",
        ...
        {
            id: "output",
            label: "Output & DLQ Routing",
            sublabel: "Serialize or Triage",
            what: "Final stage: successful sessions are normalized and serialized; anything that failed every extractor is routed to the DLQ instead of being dropped.",
            how: [
                "Successful sessions: identifiers normalized via the alias map, then enriched in-line or queued to 3rd-party lookups",
                "Failed files: logged unparseable, written to a Postgres DLQ table with full archive context",
                "DLQ entries get an LLM triage pass: new-format candidate, corrupted, or junk",
                "New-format candidates are embedded structurally and clustered to surface new stealer families",
                "Corrupted entries are tagged with a specific failure signature for human review",
                "Junk is discarded with the classification reason recorded",
            ],
            outputs: ["out.json for successful sessions", "triaged DLQ entries for clustering/review"],
        },
    ]
