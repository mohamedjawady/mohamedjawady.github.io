...
Most organizations treat threat intelligence like a feed subscription: buy a list of domains, IP addresses, and hashes, block them, and call it done. That can reduce noise, but it is not <Term concept="cti">Cyber Threat Intelligence</Term>.

<Term concept="cti">Cyber Threat Intelligence (CTI)</Term> is decision support: analyzed, contextualized findings delivered with explicit confidence and a recommended action. The key property is actionability — if a finding does not help a specific stakeholder make a better decision under uncertainty, it is data, not intelligence.

The same raw data can produce different intelligence products depending on the audience. A SOC analyst needs fast technical context for triage. An incident responder needs adversary behavior to scope and contain impact. A CISO needs risk trends, likely attacker focus, and implications for budget and strategy.

## CTI Output Schema

Use a short, consistent output schema so consumers receive the same signal shape across products. A minimal schema is:

- Assessment: one-sentence judgement
- Confidence: High / Medium / Low (see rubric below)
- Evidence: key facts and telemetry
- Alternatives: plausible competing explanations
- Action: recommended, owner, and deadline

Keeping outputs short and consistent makes them easier to automate, ingest into TIPs (MISP/OpenCTI), and map to detection workstreams.
...
Raw <Term concept="indicator-of-compromise">indicators</Term> are inputs to the analytical process. They are not the final product. An indicator becomes intelligence only when it has been contextualized, validated, enriched with adversary history, and delivered to someone who can act on it differently because of it.
...
Collect only what is relevant to the requirement. Sources may include open-source reporting, malware telemetry, <Term concept="passive-dns">passive DNS</Term>, sandbox output, dark web monitoring, or internal detection logs. The discipline is in not collecting everything. More collection does not automatically improve intelligence. It often increases noise, expands analysis workload, and delays decision support. A collector focused on a specific requirement will spend 80% of their effort on 20% of available sources and still have higher-quality input than an unfocused collection effort.
...
#### Confidence Rubric (Practical)

Use a short, reproducible rubric when assigning confidence:

- High: Multiple independent sources, corroborating telemetry, and recent validation (e.g., confirmed incident or matched telemetry within last 30 days).
- Medium: Good supporting evidence from fewer sources or partial telemetry linkage; some outstanding gaps.
- Low: Single-source reporting, uncorroborated signals, or significant untested assumptions.

When writing confidence, include the top 2 supporting facts and one major gap that would reduce confidence if disproven.
...
For machine-readable transport and structured interchange, use STIX/TAXII and Threat Intelligence Platforms (TIPs) such as MISP or OpenCTI. These tools do not replace analytic judgement, but they make automated dissemination, enrichment, and ingestion into detection pipelines far more reliable.
...
## Quick Contrast: Bad CTI vs Good CTI

A short side-by-side helps clarify what to avoid and what to aim for.

Bad CTI (what you see in most feed dumps)

- Format: Long email or CSV of indicators
- Content: 200 hashes, no context, no confidence, no recommended owner
- Actionability: SOC must triage everything; high noise, low utility

Good CTI (what a consumer can action)

- Format: One-page assessment + machine-readable artifact
- Content: One-sentence assessment, confidence, top evidence, ATT&CK mapping, recommended Sigma/YARA rule and owner
- Actionability: SOC applies one targeted detection; hunting and leadership get prioritized tasks
...
**Collection:** Query <Term concept="shodan">Shodan</Term>, dark web monitors, and breach databases for your organization's domain name or email patterns. Search threat forums for credential dumps posted in the past 30 days. Query your own SIEM for failed VPN login attempts using internal user credentials from external IPs.
...
**Dissemination (Tactical):** Deploy specific <Term concept="sigma">Sigma</Term> rules for <Term concept="fin7">FIN7</Term> credential dumping patterns. Tune <Term concept="edr">EDR</Term> policies for <Term concept="living-off-the-land">living-off-the-land</Term> binary detection.
...
Audience: <Term concept="soc">SOC</Term> analysts, detection engineers, IR teams. Tactical intelligence is the most granular: <Term concept="yara">YARA</Term> rules, <Term concept="sigma">Sigma</Term> detections, specific hashes, IP ranges to block, <Term concept="c2">C2</Term> communication patterns. This is what a SOC analyst or an automated detection system ingests directly into defensive tooling. It requires no further analysis. It is immediately operationalizable.
